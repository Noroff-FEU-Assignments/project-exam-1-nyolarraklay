...
const limitPage = "?per_page=9";

const container = document.querySelector(".blogsContainerCarousel");
...
async function getBlogs() {
  try {
    const response = await fetch(blogsUrl + limitPage);
...
function createBlogHTML(blog) {
  try {
    ...
    blogContainer.classList.add("blogContainerCarousel");
